...
CRPCResp::CRPCResp() : strJSONRPC("2.0") {}
...
bool CRPCResp::IsSuccessful() const { return (bool)spResult; }
...
CRPCRespVec DeserializeCRPCResp(const CRPCReqMap& req, const std::string& str)
{
    ...
    if (valResponse.type() != json_spirit::array_type &&
        valResponse.type() != json_spirit::obj_type)
    {
        throw CRPCException(
            RPC_PARSE_ERROR,
    ...
    {
        const json_spirit::Object& response = 
            (valResponse.type() == json_spirit::array_type) 
                ? valResponse.get_array()[i].get_obj() : valResponse.get_obj();
        ...
        else if (valError.type() == json_spirit::obj_type &&
                 valResult.is_null())
        {
