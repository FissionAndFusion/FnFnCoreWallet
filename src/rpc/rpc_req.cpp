...
CRPCReq::CRPCReq() : strJSONRPC("2.0") {}
...
CRPCReqVec DeserializeCRPCReq(const std::string& str, bool& fArray)
{
    ...
    if (valRequest.type() != json_spirit::array_type &&
        valRequest.type() != json_spirit::obj_type)
    {
        throw CRPCException(RPC_PARSE_ERROR,
    ...
    {
        const json_spirit::Object& request =
            (valRequest.type() == json_spirit::array_type)
        ...
        if (req->valID.type() != json_spirit::int_type &&
            req->valID.type() != json_spirit::str_type && !req->valID.is_null())
        {
