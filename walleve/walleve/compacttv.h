...
#ifndef  WALLEVE_COMPACTTV_H
#define  WALLEVE_COMPACTTV_H
...
class CWalleveCompactTagValue
{
public:
    typedef boost::variant<boost::int64_t,std::vector<unsigned char> > CWalleveCompactTVType;
    enum
    {
        WALLEVE_COMPACT_TYPE_FALSE  = (0 << 5),
        WALLEVE_COMPACT_TYPE_TRUE   = (1 << 5),
        WALLEVE_COMPACT_TYPE_INT8   = (2 << 5),
        WALLEVE_COMPACT_TYPE_INT16  = (3 << 5),
        WALLEVE_COMPACT_TYPE_INT32  = (4 << 5),
        WALLEVE_COMPACT_TYPE_INT64  = (5 << 5),
        WALLEVE_COMPACT_TYPE_INT256 = (6 << 5),
        WALLEVE_COMPACT_TYPE_BINARY = (7 << 5),
    };

    CWalleveCompactTagValue() {}
    void Push(int nTag,const std::vector<unsigned char>& vch,const boost::false_type&)
    {
        ...
        mapTagValue.insert(std::make_pair(nTag,std::make_pair(WALLEVE_COMPACT_TYPE_BINARY,vch)));
    }
    ...
    {
        int type = (f ? WALLEVE_COMPACT_TYPE_TRUE : WALLEVE_COMPACT_TYPE_FALSE);
    ...
    void Push(int nTag,const T& t,const boost::false_type&)
    {
        ...
        mapTagValue.insert(std::make_pair(nTag,std::make_pair(WALLEVE_COMPACT_TYPE_INT256,vch)));
    }
    template <typename T>
    void Push(int nTag,const T& t,const boost::true_type&)
    {
        ...
        {
        case 1:
            type = WALLEVE_COMPACT_TYPE_INT8;
            break;
        case 2:
            type = WALLEVE_COMPACT_TYPE_INT16;
            break;
        case 4:
            type = WALLEVE_COMPACT_TYPE_INT32;
            break;
        case 8:
            type = WALLEVE_COMPACT_TYPE_INT64;
            break;
        default:
            throw std::range_error("unknown type");
            break;
    ...
    {
        std::map<int,std::pair<int,CWalleveCompactTVType> >::iterator it = mapTagValue.find(nTag);
        if (it == mapTagValue.end())
        {
            return false;
        }
        if ((*it).second.first != WALLEVE_COMPACT_TYPE_BINARY)
        {
    ...
    bool Get(int nTag,bool& f)
    {
        ...
        if ((*it).second.first != WALLEVE_COMPACT_TYPE_TRUE && (*it).second.first != WALLEVE_COMPACT_TYPE_FALSE)
        {
            throw std::range_error("type error");
        }
        f = ((*it).second.first == WALLEVE_COMPACT_TYPE_TRUE);
        return true;
    }
    template <typename T>
    bool Get(int nTag,T& t)
    {
        ...
        if ((type == WALLEVE_COMPACT_TYPE_INT256 && sizeof(T) != 32) 
            || (type != WALLEVE_COMPACT_TYPE_INT256 && sizeof(T) > 8))
        {
            throw std::range_error("type error");
        }
        if (type == WALLEVE_COMPACT_TYPE_INT256)
        {
            std::vector<unsigned char>& vch = boost::get<std::vector<unsigned char> >((*it).second.second);
            t = *(T*)(&vch[0]);
        }
    ...
    void Encode(std::vector<unsigned char>& vchEncode)
    {
             ...
             it != mapTagValue.end();++it)
        {
            ...
            else
            {
                ...
                switch(type)
                {
                case WALLEVE_COMPACT_TYPE_INT8:
                    len = 1;
                    break;
                case WALLEVE_COMPACT_TYPE_INT16:
                    len = 2;
                    break;
                case WALLEVE_COMPACT_TYPE_INT32:
                    len = 4;
                    break;
                case WALLEVE_COMPACT_TYPE_INT64:
                    len = 8;
                    break;
                default:
                    break;
    ...
    void Decode(const std::vector<unsigned char>& vchEncoded)
    {
        ...
        while (offset != vchEncoded.size())
        {
            ...
            if (type == WALLEVE_COMPACT_TYPE_BINARY)
            {
                ...
                if (!mapTagValue.insert(std::make_pair(tag,std::make_pair(WALLEVE_COMPACT_TYPE_BINARY,vch))).second)
                {
                    throw std::range_error("duplicated tag");
                }
                offset += size;
            ...
            else if (type == WALLEVE_COMPACT_TYPE_INT256)
            {
                ...
                if (!mapTagValue.insert(std::make_pair(tag,std::make_pair(WALLEVE_COMPACT_TYPE_INT256,vch))).second)
                {
                    throw std::range_error("duplicated tag");
                }
                offset += 32;
...
protected:
    std::map<int,std::pair<int,CWalleveCompactTVType> > mapTagValue;
};

} // namespace walleve

#endif //WALLEVE_COMPACTTV_H
