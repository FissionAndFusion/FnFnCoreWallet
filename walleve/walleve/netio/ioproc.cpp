...
: IIOProc(walleveOwnKeyIn),
  thrIOProc(walleveOwnKeyIn,boost::bind(&CIOProc::IOThreadFunc,this)), 
...
bool CIOProc::WalleveHandleInvoke()
{
...
void CIOProc::WalleveHandleHalt()
{
...
uint32 CIOProc::SetTimer(uint64 nNonce,int64 nElapse)
{
    ...
    it = mapTimerById.insert(make_pair(nTimerId,CIOTimer(nTimerId,nNonce,WalleveGetTime() + nElapse))).first;
...
void CIOProc::IOProcHandleEvent(CWalleveEvent * pEvent,shared_ptr<CIOCompletion> spComplt)
{
